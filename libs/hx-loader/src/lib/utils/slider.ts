...
import { success } from '@hx-ctrl/hx-loader';
import fs from 'fs';
import { error } from 'libs/hx-loader/src/lib/utils/process';
...
export const generatePlugin = (sliders: Slider[], config: Config) => {
  ...
  outputData = sliders.reduce((_outputData, s, i) => {
    ...
    return `${_outputData}slider${id}:${s.default}<${s.min},${s.max},1>${
      s.cc ? `(${s.cc}) ` : ''
    }${s.title}\n`;
  }, outputData);
...
@block\n`;
 ...
 c${id}_run = 0;
  ...
  if (!fs.existsSync(outputPath)) {
    try {
      const asdf = fs.mkdirSync(outputPath, { recursive: true });
      console.log('#117', asdf);
